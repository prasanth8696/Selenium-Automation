    ...
    try :
        ...
        while True :
            ...
            if taskNumber :
                ...
                if not taskDetails :
                    ...
                    if nextRecordBtnTagTitle == "Bottom of list displayed" or isNextRecordBtnTagDisabled == "true" :
                        logger.info("this is the last record so break the loop")
                        validatedRowCount += 1
                        break
                    #increase the count
                    validatedRowCount += 1
                    #click the next record button to get the nect record
            ...
            if (totalAssetCountInString > 50) :
                ...
                logger.debug(f"total non remdiated sting machine count {totalAssetCountInString} is more than or equal to 50 ")
            ...
            validatedRowCount += 1
            logger.info(f"{taskNumber} validated sucessfully - {validatedRowCount}/{totalRows}")


            if nextRecordBtnTagTitle == "Bottom of list displayed" or isNextRecordBtnTagDisabled == "true" :
                
                logger.debug("this is the bottom of the list so breaking the loop")
                break
            else:
                #get the next record
                logger.info("getting the next record")
